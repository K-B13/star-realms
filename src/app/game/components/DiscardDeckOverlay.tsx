...
interface DiscardDeckOverlayProps {
...
export default function DiscardDeckOverlay({currentPlayer, onClose}: DiscardDeckOverlayProps) {
    const playedCards = [...currentPlayer.discard, ...currentPlayer.inPlay];
    ...
    return (
        ...
        >
            <div 
                className="bg-gradient-to-br from-slate-800 to-slate-900 border-3 border-purple-500 rounded-xl shadow-2xl shadow-purple-500/30 p-6 max-w-4xl w-full max-h-[90vh] flex flex-col"
                onClick={(e) => e.stopPropagation()}
            >
                {/* Header */}
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-2xl font-bold text-purple-300">Discard Pile</h3>
                    <button
                        onClick={onClose}
                ...
                <p className="text-gray-400 text-sm mb-4">
                    {playedCards.length} {playedCards.length === 1 ? 'card' : 'cards'}
                </p>
                ...
                <div className="flex-1 overflow-y-auto">
                    {playedCards.length === 0 ? (
                        <div className="flex items-center justify-center h-full">
                            <p className="text-gray-500 text-lg">No cards in discard pile</p>
                        </div>
                    ) : (
                ...
                <button
                    onClick={onClose}
                    className="mt-4 border-2 border-purple-500 bg-purple-900/40 hover:bg-purple-800/60 text-purple-300 px-6 py-2 rounded-lg font-semibold transition-all w-full"
                >
                    Close
                </button>
            </div>
